...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM minibuslle
  ...
  // the value of the form is $date,$period
    ...
    if ($name) {
      ...
      $sqlGetDescription = "SELECT BookingDesc FROM minibuslle
                            WHERE BookingDate=$date AND BookingPeriod=$period";
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          if(canBookVehicles($conn)) {
            ...
            echo "<form method='POST' action='minibuslle.php' id=$date,$period>";
            ...
            echo "onclick=\"getBookingDescription($date,$period); return confirm('" . writeDialog("Minibus LLE",$date,$period) . "');\"";
  ...
  function createBooking($inputstring,$bookingdesc,$conn) {
    ...
    try {
      ...
      echo "<form method='POST' action='minibuslle.php'>";
  ...
  <center>
  <h3>Minibus LLE</h3><br>         
  <form method="POST" action='minibuslle.php'>
