...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM meetingroom
  ...
  // the value of the form is $date,$period
    ...
    if ($name) {
      ...
      $sqlGetDescription = "SELECT BookingDesc FROM meetingroom
                            WHERE BookingDate=$date AND BookingPeriod=$period";
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          ...
          echo "<form method='POST' action='meetingroom.php' id=$date,$period>";
          ...
          echo "onclick=\"getBookingDescription($date,$period); return confirm('" . writeDialog("Meeting Room",$date,$period) . "');\"";
  ...
  function createBooking($inputstring,$bookingdesc,$conn) {
    ...
    try {
      ...
      echo "<form method='POST' action='meetingroom.php'>";
  ...
  <center>
  <h3>Meeting Room</h3><br>  
  <form method="POST" action='meetingroom.php'>
