...
<html>
<head>
  <title>Phonology Screener</title>
  <!--CSS stylesheet-->
...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM slt_screener
        ...
        if (checkIfInThePast($date)) {
          ...
          if(!readOnly($conn)) {
            if(canBookSLT($conn)) {
              ...
              echo "<form method='POST' action='slt_screener.php'>";
              echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
              echo "onclick=\"return confirm('" . writeDialog("Phonology Screener",$date,$period) . "');\"";
  ...
  <center>
  <h3>Phonology Screener</h3><br>
  <form method="POST" action='slt_screener.php'>
