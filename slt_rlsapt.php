...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM slt_rlsapt
          ...
          if (checkIfInThePast($date)) {
            ...
            if(!readOnly($conn)) {
              if(canBookSLT($conn)) {
                ...
                echo "<form method='POST' action='slt_rlsapt.php'>";
                echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
                echo "onclick=\"return confirm('" . writeDialog("RLS APT",$date,$period) . "');\"";
  ...
  <center>
  <h3>RLS APT</h3><br>
  <form method="POST" action='slt_rlsapt.php'>
