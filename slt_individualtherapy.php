...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM slt_individualtherapy
        ...
        if (checkIfInThePast($date)) {
          ...
          if(!readOnly($conn)) {
            if(canBookSLT($conn)) {
              ...
              echo "<form method='POST' action='slt_individualtherapy.php'>";
              echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
              echo "onclick=\"return confirm('" . writeDialog("Individual Therapy",$date,$period) . "');\"";
  ...
  <center>
  <h3>Individual Therapy</h3><br>         
  <form method="POST" action='slt_individualtherapy.php'>
  ...
  <?php
    if (!canBookSLT($conn)) {
      echo "<b>You are not permitted to book Speech and Language resources</b></br>";
    }
  ?>
  </center>
