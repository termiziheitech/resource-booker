...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM ipadtrolleygrey
    ...
    if ($name) {
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          ...
          echo "<form method='POST' action='ipadtrolleygrey.php'>";
          echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
          echo "onclick=\"return confirm('" . writeDialog("iPad Trolley Grey",$date,$period) . "');\"";
  ...
  <center>
  <h3>iPad Trolley (Grey)</h3><br>
  <form method="POST" action='ipadtrolleygrey.php'>
