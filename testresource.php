...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM testresource
    ...
    if ($name) {
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          ...
          echo "<form method='POST' action='testresource.php'>";
          echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
          echo "onclick=\"return confirm('" . writeDialog("Test Resource",$date,$period) . "');\"";
  ...
  <center>
  <h3>Test Resource</h3><br>         
  <form method="POST" action='testresource.php'>
