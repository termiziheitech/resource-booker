...
<head>
  ...
  function getBookingDescription(date, period) {
    var bookingDesc = prompt("Please write a short description of the booking");
    if (bookingDesc != null) {
      document.getElementById("hiddenbooking,"+ date + "," + period).value = bookingDesc;
  ...
  $(document).ready(function(){
    $(".booked").hover(
      function(){
        ...
        $(this).find(".bookedtext").text($description);
      }, function() {
        var mytext = "testing";
...
<body>
 ...
 <?php
  ...
  // This determines if they are allowed to book anything or not
  function isReadOnly() {
  ...
  function writeCell($conn,$date,$period) {
    ...
    if ($name) {
      ...
      $sqlGetDescription = "SELECT BookingDesc FROM conferenceroom
                            WHERE BookingDate=$date AND BookingPeriod=$period";
      ...
      echo "<b><td class='booked'><span class='bookedtext' style='font-weight:bold'>".$name."</a></span>
            <input type='hidden' id='description' value=\"$bookingdesc\"' /></td></b>"; //  href='#' onClick='alert(\"" . writeDescription($bookingdesc) ."\")'
    } else {
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          ...
          echo "<form method='POST' action='conferenceroom.php' id=$date,$period>";
          ...
          echo "onclick=\"getBookingDescription($date,$period); return confirm('" . writeDialog("Conference Room",$date,$period) . "');\"";
  ...
  // Function for time-based bookings (Conference Room and Meeting Room)
  // Since the SQL table uses period system, rooms have available periods 1-8
  // where period 1 represents timeslot 9:00AM - 10:00AM, etc.
  // This function converts period to timeslot
  // * To be expanded to include AM/PM as well as 24hr (based on user preference?)
  function writeTimeSlot($period) {
    // very hacky solution, where you return 'period+8':00 - 'period+9':00
    // so period 1 gives '9:00 - 10:00'; 
    return ($period+8).":00 - ".($period+9).":00";
  }// writeTimeSlot
  ...
  function createBooking($inputstring,$bookingdesc,$conn) {
    //$conn = $_SESSION["conn"];
    ...
    try {
      ...
      echo "<form method='POST' action='conferenceroom.php'>";
  ...
  <center>
  <h3>Conference Room</h3><br>
  <form method="POST" action='conferenceroom.php'>
