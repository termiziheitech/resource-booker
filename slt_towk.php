...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM slt_towk
...
function writeCell($conn,$date,$period) {
        ...
        if (checkIfInThePast($date)) {
          ...
          if(!readOnly($conn)) {
            if(canBookSLT($conn)) {
              ...
              echo "<form method='POST' action='slt_towk.php'>";
              echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
              echo "onclick=\"return confirm('" . writeDialog("TOWK",$date,$period) . "');\"";
  ...
  <center>
  <h3>TOWK</h3><br>
  <form method="POST" action='slt_towk.php'>
