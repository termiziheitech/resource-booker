...
<head>
  ...
  <!--CSS stylesheet-->
  <link rel='stylesheet' type='text/css' href='my-style-all-landscape.css'>
  <!--JQuery library-->
 ...
 <?php
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          if(canBookVehicles($conn)) {
            ...
            echo "<form method='POST' action='all_vehicles_landscape.php' id=$date,$period,$table>";
  ...
  function createBooking($inputstring,$bookingdesc,$conn,$table) {
    ...
    try {
      ...
      echo "<form method='POST' action='all_vehicles_landscape.php'>";
  ...
  <form method="GET" action='all_vehicles_landscape_print.php' target="_blank">
    <button type='submit' id='printweek' name='printweek'<?php echo "value='".$_SESSION['mondate']."'"; ?>>Print This Week</button><br>
  </form>      
  <form method="POST" action='all_vehicles_landscape.php'>
