...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM slt_rlswfvt
        ...
        if (checkIfInThePast($date)) {
          ...
          if(!readOnly($conn)) {
            if(canBookSLT($conn)) {
              ...
              echo "<form method='POST' action='slt_rlswfvt.php'>";
              echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
              echo "onclick=\"return confirm('" . writeDialog("RLS WFVT",$date,$period) . "');\"";
  ...
  <center>
  <h3>RLS WFVT</h3><br>
  <form method="POST" action='slt_rlswfvt.php'>
