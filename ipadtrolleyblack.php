...
<html>
<head>
  <title>iPad Trolley (Black)</title>
  <!--CSS stylesheet-->
...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM ipadtrolleyblack
    ...
    if ($name) {
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          ...
          echo "<form method='POST' action='ipadtrolleyblack.php'>";
          echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
          echo "onclick=\"return confirm('" . writeDialog("iPad Trolley Black",$date,$period) . "');\"";
  ...
  <center>
  <h3>iPad Trolley (Black)</h3><br>         
  <form method="POST" action='ipadtrolleyblack.php'>
