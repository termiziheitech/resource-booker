...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM slt_talc1
...
function writeCell($conn,$date,$period) {
        ...
        if (checkIfInThePast($date)) {
          ...
          if(!readOnly($conn)) {
            if(canBookSLT($conn)) {
              ...
              echo "<form method='POST' action='slt_talc1.php'>";
              echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
              echo "onclick=\"return confirm('" . writeDialog("TALC 1",$date,$period) . "');\"";
  ...
  <center>
  <h3>TALC 1</h3><br>
  <form method="POST" action='slt_talc1.php'>
