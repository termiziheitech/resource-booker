...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM slt_ndp
          ...
          if (checkIfInThePast($date)) {
            ...
            if(!readOnly($conn)) {
              if(canBookSLT($conn)) {
                ...
                echo "<form method='POST' action='slt_ndp.php'>";
                echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
                echo "onclick=\"return confirm('" . writeDialog("NDP",$date,$period) . "');\"";
  ...
  <center>
  <h3>NDP</h3><br>
  <form method="POST" action='slt_ndp.php'>
