...
<body>
      ...
      <li class="makeabooking" style="border-left:1px solid #DDDDDD;"><a href="#">Make a Booking</a>
        <ul>
          ...
          <li><a href="#">Vehicles &raquo;</a>            
            <ul>
              <li><a href="/all_vehicles_landscape.php">All Vehicles</a></li>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM minibusjbx
  ...
  // the value of the form is $date,$period
    ...
    if ($name) {
      ...
      $sqlGetDescription = "SELECT BookingDesc FROM minibusjbx
                            WHERE BookingDate=$date AND BookingPeriod=$period";
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          if(canBookVehicles($conn)) {
            ...
            echo "<form method='POST' action='minibusjbx.php' id=$date,$period>";
            ...
            echo "onclick=\"getBookingDescription($date,$period); return confirm('" . writeDialog("Minibus JBX",$date,$period) . "');\"";
  ...
  function createBooking($inputstring,$bookingdesc,$conn) {
    ...
    try {
      ...
      echo "<form method='POST' action='minibusjbx.php'>";
  ...
  <center>
  <h3>Minibus JBX</h3><br>         
  <form method="POST" action='minibusjbx.php'>
