...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM slt_acap
        ...
        if (checkIfInThePast($date)) {
          ...
          if(!readOnly($conn)) {
            if(canBookSLT($conn)) {
              ...
              echo "<form method='POST' action='slt_acap.php'>";
              echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
              echo "onclick=\"return confirm('" . writeDialog("ACAP",$date,$period) . "');\"";
  ...
  <center>
  <h3>ACAP</h3><br>
  <form method="POST" action='slt_acap.php'>
