...
<body>
 ...
 <?php
        ...
        if (checkIfInThePast($date)) {
          ...
          if(!readOnly($conn)) {
            if(canBookSLT($conn)) {
              ...
              echo "<form method='POST' action='slt_grouptherapy.php'>";
              echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
              echo "onclick=\"return confirm('" . writeDialog("Group Therapy",$date,$period) . "');\"";
  ...
  <center>
  <h3>Group Therapy</h3><br>         
  <form method="POST" action='slt_grouptherapy.php'>
