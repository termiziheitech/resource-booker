...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM minibusjha
  ...
  // the value of the form is $date,$period
    ...
    if ($name) {
      ...
      $sqlGetDescription = "SELECT BookingDesc FROM minibusjha
                            WHERE BookingDate=$date AND BookingPeriod=$period";
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          if(canBookVehicles($conn)) {
            ...
            echo "<form method='POST' action='minibusjha.php' id=$date,$period>";
            ...
            echo "onclick=\"getBookingDescription($date,$period); return confirm('" . writeDialog("Minibus JHA",$date,$period) . "');\"";
  ...
  function createBooking($inputstring,$bookingdesc,$conn) {
    ...
    try {
      ...
      echo "<form method='POST' action='minibusjha.php'>";
  ...
  <center>
  <h3>Minibus JHA</h3><br>         
  <form method="POST" action='minibusjha.php'>
