...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM testroom
  ...
  function writeCell($conn,$date,$period) {
    ...
    if ($name) {
      ...
      $sqlGetDescription = "SELECT BookingDesc FROM testroom
                            WHERE BookingDate=$date AND BookingPeriod=$period";
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          ...
          echo "<form method='POST' action='testroom.php' id=$date,$period>";
          ...
          echo "onclick=\"getBookingDescription($date,$period); return confirm('" . writeDialog("Test Room",$date,$period) . "');\"";
  ...
  function createBooking($inputstring,$bookingdesc,$conn) {
    ...
    try {
      ...
      echo "<form method='POST' action='testroom.php'>";
  ...
  <center>
  <h3>Test Room</h3><br>
  <form method="POST" action='testroom.php'>
