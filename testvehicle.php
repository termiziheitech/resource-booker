...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM testvehicle
    ...
    if ($name) {
      ...
      if (checkIfInThePast($date)) {
        ...
        if(!readOnly($conn)) {
          ...
          echo "<form method='POST' action='testvehicle.php'>";
          echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
          echo "onclick=\"return confirm('" . writeDialog("Test Vehicle",$date,$period) . "');\"";
  ...
  <center>
  <h3>Test Vehicle</h3><br>         
  <form method="POST" action='testvehicle.php'>
