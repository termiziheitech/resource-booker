...
<body>
 ...
 <?php
  ...
  function checkAvailable($conn,$date,$period) {
    $sql = "SELECT BookedByName FROM slt_dls
        ...
        if (checkIfInThePast($date)) {
          ...
          if(!readOnly($conn)) {
            if(canBookSLT($conn)) {
              ...
              echo "<form method='POST' action='slt_dls.php'>";
              echo "<td class='available'><button type='submit' id='availablebtn' name='book'";
              echo "onclick=\"return confirm('" . writeDialog("DLS",$date,$period) . "');\"";
  ...
  <center>
  <h3>DLS</h3><br>
  <form method="POST" action='slt_dls.php'>
